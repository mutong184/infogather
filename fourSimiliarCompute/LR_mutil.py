...
class LogisticRegressionModel:
    ...
    def train(self, x_train, y_train, epochs=50, batch_size=10):
        ...
        self.model.compile(optimizer='adam', loss=self.mean_absolute_error)
...
def train_mode(train_file,model_store):
    ...
    weigthfile = os.path.join(path,"weigth_multi.txt")
    ...
    # 计算准确度
    accuracy = tf.reduce_mean(tf.abs(predictions - y_train))
    print("平均精度：", accuracy)
...
def predict(test_file,model_path):
    ...
    print("准确度：", accuracy)
train_mode('D:\ljj\data\\traindata.txt',"D:\ljj\data\model_muti")
print("train success")
predict('D:\ljj\data\\testdata.txt',"D:\ljj\data\model_muti")